...
Trong repo hiện tại, branch `testing` đang ahead `origin/testing`. Nếu `origin` trỏ tới repo bạn không có quyền ghi, push sẽ lỗi. Cách an toàn là đặt `origin` về fork của bạn và giữ repo gốc làm `upstream`.

Với repo của bạn là `https://github.com/daithang59/docvault.git`:
...
Nếu remote `upstream` hiện đang là repo bạn có quyền ghi, có thể push thử:

```bash
git push --dry-run upstream testing
git push -u upstream testing
```

Không push trực tiếp vào repo của người khác nếu bạn không chắc mình có quyền maintainer.
...
2. Branch code, ví dụ `testing`.
3. Branch GitOps, ví dụ `gitops-testing`.
...
Kiểm tra `@Library('docvault@testing')` và Global Trusted Pipeline Library tên `docvault`.
...
1. Sửa Git remote và push được branch `testing`.
