...
Trong repo hiện tại `dockerOrg` vẫn đang là `duyimew`. Trước khi chạy tới bước push image, đổi giá trị này sang Docker Hub namespace bạn sở hữu, ví dụ `daithang59` nếu Docker Hub của bạn dùng đúng tên đó.
...
Lưu ý quan trọng: các fix pipeline hiện đang nằm trên branch `devsecops-pipeline`. Nếu bạn chạy Jenkins trước khi merge vào `testing`, hãy đổi tạm dòng trên thành:
