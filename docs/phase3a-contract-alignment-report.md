...
| `features/documents/documents.api.presignDownload` / stream flow | `POST /documents/:docId/presign-download`, `GET /documents/:docId/versions/:version/stream` | same | `presign-download`: `{ version?, grantToken? }`; `stream`: path param version plus optional `token` query | `{ url, expiresAt, ... }` / binary stream | viewer/editor/approver/admin; compliance_officer denied | `fixed` | Backend accepts a pre-authorized `grantToken` or re-authorizes internally when it is omitted. Stream tokens are bound to actor, document id, and version. |
