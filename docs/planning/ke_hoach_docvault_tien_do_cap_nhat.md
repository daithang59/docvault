...
**Trạng thái hiện tại:** Đã có Argo CD Application tại `infra/argocd-apps/monitoring.yaml` cho `kube-prometheus-stack` và `infra/argocd-apps/loki.yaml` cho Loki/promtail, đều bật auto-sync/self-heal và đã apply lên EKS. App `monitoring-stack` và `loki-stack` đã đạt `Synced/Healthy`; Grafana, Prometheus, Alertmanager, Loki và promtail đã Running. Loki đã nhận log từ namespace `docvault`. Việc còn lại là chụp dashboard metrics và màn hình Grafana Explore logs làm bằng chứng.
...
| Dependency Check SCA | Đã tích hợp gate CVSS >= 7 | Medium/Low được triage bằng exception record |
...
Hiện tại đã có bằng chứng ZAP chạy được trên target EKS. Lần chạy đầu với `ZAP_TARGET=http://<node-ip>:30006/api` sinh report nhưng có spider warning vì `/api` trả 404. Baseline scan nên chạy lại với `ZAP_TARGET=http://<node-ip>:30006`; code scan, target reachability check, report output, artifact archive và gate High/Critical theo `zap_report.json` đã có trong pipeline.

### 7.2. SCA còn cảnh báo

Dependency Check đã chạy nhưng có cảnh báo. Nhóm đã tạo bản ghi triage tại `docs/devsecops/security-sca-triage.md`, nâng cấp một số dependency trực tiếp và pin override cho các dependency transitive có bản vá phù hợp. Trước demo cần tải artifact Jenkins mới và đối chiếu lại với bản ghi triage này.

### 7.3. Observability chưa triển khai

Đã triển khai kube-prometheus-stack và Loki/promtail qua Argo CD. App `monitoring-stack` và `loki-stack` đã `Synced/Healthy`; Loki đã nhận log từ namespace `docvault`. Chưa hoàn thiện bằng chứng ảnh; cần mở Grafana và chụp dashboard vận hành cùng màn hình Explore logs.
...
- Policy demo đã được implement: ZAP warning không làm fail MVP demo, nhưng pipeline vẫn fail nếu không sinh được report hoặc `zap_report.json` có High/Critical finding.
...
- `infra/argocd-apps/loki.yaml` đã được apply và bật auto-sync/self-heal.
- Argo CD app `monitoring-stack` và `loki-stack` đã ở trạng thái Synced/Healthy.
- Mở Grafana bằng port-forward.
- Chụp dashboard pod health, CPU/RAM, workload status và Grafana Explore logs với query `{namespace="docvault"}`.
...
- Đối chiếu với `docs/devsecops/security-sca-triage.md`.
- Phân loại cảnh báo theo Critical/High/Medium/Low.
...
- Script demo EKS tại `docs/getting-started/demo-flow.md`.
- Video demo ngắn.
...
- [x] Có Loki/promtail.
- [x] Có log theo service trong Loki.
...
4. Đối chiếu Dependency Check artifact với `docs/devsecops/security-sca-triage.md`.
