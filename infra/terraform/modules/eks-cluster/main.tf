...
locals {
  cluster_creator_admin_principal_arn = "arn:${data.aws_partition.current.partition}:iam::${data.aws_caller_identity.current.account_id}:root"
}
...
module "eks" {
  ...
  access_entries = {
    cluster_creator = {
      principal_arn = local.cluster_creator_admin_principal_arn
      type          = "STANDARD"
